    ...
    static {
        try {
            ...
            props.load(new InputStreamReader(
                    OnlovProperties.class.getClassLoader().getResourceAsStream("application.properties"),
                    "UTF-8"));
            String profile = props.getProperty("spring.profiles.active");
            String envFile= "application-" + profile + ".properties";

            Properties envProps = new Properties();
            envProps.load(new InputStreamReader(OnlovProperties.class.getClassLoader().getResourceAsStream(envFile), "UTF-8"));
            Field[] fields = OnlovProperties.class.getFields();
